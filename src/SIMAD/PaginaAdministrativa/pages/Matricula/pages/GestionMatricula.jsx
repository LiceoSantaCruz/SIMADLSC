...
// Se importa la función utilitaria para generar el Comprobante (sin logo)
import { generateComprobanteMatriculaPDF } from "../Utils/pdfComprobanteMatricula";

const API_URL = import.meta.env.VITE_API_URL;


// Modal con pestañas para detalles de la matrícula
            ...
            {docs.length === 0 ? (
              ...
              <ul className="list-disc list-inside">
                {docs.map((fileName, idx) => {
                  const url = `${API_URL}/uploads/est-${stu.cedula}/${fileName}`;
                  return (
...
InfoModal.propTypes = {
  matricula: PropTypes.shape({
    ...
    encargadoLegal: PropTypes.object,
    seccion: PropTypes.object,
    archivos: PropTypes.arrayOf(PropTypes.string),
  }).isRequired,
...
export default function GestionMatricula() {
  ...
  // Construir arrays de niveles y estados únicos
  const niveles = Array.from(
    new Set(matriculas.map((m) => m.estudiante?.grado?.nivel).filter((n) => n))
  );
  const estados = Array.from(new Set(matriculas.map((m) => m.estado_Matricula)));

  // Filtrado por nivel, estado y texto de búsqueda (cédula)
  const filtered = matriculas.filter((m) => {
    const nivel = m.estudiante?.grado?.nivel;
    return (
      (!selectedNivelFilter || nivel === selectedNivelFilter) &&
      (!selectedEstadoFilter || m.estado_Matricula === selectedEstadoFilter) &&
      (m.estudiante?.cedula?.includes(searchTerm) ?? false)
    );
  });

  // Ordenamiento por prioridad de estado: RE (2), AC (3), resto (1)
  const sorted = [...filtered].sort(
    (a, b) =>
      ({ RE: 2, AC: 3 }[a.estado_Matricula] || 1) -
      ({ RE: 2, AC: 3 }[b.estado_Matricula] || 1)
  );
  ...
  // Lógica para mostrar hasta 6 botones de página
  const maxBtn = 6;
  let startPage = 1,
    endPage = totalPages;
  if (totalPages > maxBtn) {
    ...
    if (currentPage <= half) endPage = maxBtn;
      ...
      startPage = totalPages - maxBtn + 1;
    else {
      startPage = currentPage - half;
      endPage = startPage + maxBtn - 1;
    }
  }
          ...
          <tbody>
            {pageItems.map((m, idx) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                  {m.estudiante?.apellido1_Estudiante}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                  {m.estudiante?.apellido2_Estudiante}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                  {m.estudiante?.nombre_Estudiante}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                  {m.estudiante?.cedula}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                  {m.estudiante?.grado?.nivel ?? "N/A"}
                </td>
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                  {m.estado_Matricula}
                </td>
                <td className="px-6 py-4 flex space-x-2">
                  ...
                  {/* Botón “Aceptar” */}
                  {m.estado_Matricula !== "AC" &&
                    m.estado_Matricula !== "RE" && (
                      <button
                        onClick={() =>
                          handleUpdateEstado(m.id_Matricula, "AC")
                        }
                  ...
                  {m.estado_Matricula !== "RE" &&
                    m.estado_Matricula !== "AC" && (
                      <button
                        onClick={() =>
                          handleUpdateEstado(m.id_Matricula, "RE")
                        }
                  ...
                  {m.estado_Matricula === "RE" && (
                    <button
                      onClick={() => showDeleteConfirm(m.id_Matricula)}
                  ...
                  {/* Botón “Comprobante” (solo si AC) */}
                  {m.estado_Matricula === "AC" && (
                    <button
                      onClick={() => generateComprobanteMatriculaPDF(m)}
                      className="px-2 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700 whitespace-nowrap"
                    >
                      Comprobante
                    </button>
                  )}
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex justify-center space-x-2">
          <button
            ...
            className="px-3 py-1 bg-blue-100 dark:bg-blue-900 rounded disabled:opacity-50"
          >
            &lt;
          </button>
          {Array.from(
...
GestionMatricula.propTypes = {
  // Puedes agregar PropTypes si este componente recibe props externos
};
