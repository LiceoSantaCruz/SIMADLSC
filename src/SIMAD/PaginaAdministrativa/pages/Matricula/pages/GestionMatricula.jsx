...
const API_URL = import.meta.env.VITE_API_URL; // asegúrate de tener VITE_API_URL en tu .env
...
export default function GestionMatricula() {
  ...
  }).then(res => res.isConfirmed && (handleDelete(id), Swal.fire({
    icon: "success",
    title: "Eliminada",
    confirmButtonColor: "#2563EB"
  })));
  ...
  const niveles = Array.from(new Set(
    matriculas.map(m => m.estudiante?.grado?.nivel).filter(n => n)
  ));
  const estados = Array.from(new Set(
    matriculas.map(m => m.estado_Matricula)
  ));

  const filtered = matriculas.filter(m => {
    const nivel = m.estudiante?.grado?.nivel;
    return (!selectedNivelFilter || nivel === selectedNivelFilter)
    ...
    <div className="p-4">
      ...
      <div className="flex flex-nowrap gap-4 mb-4">
        ...
        <select
          ...
          onChange={e => { setSelectedNivelFilter(e.target.value); setCurrentPage(1); }}
          className="border p-2 rounded bg-white dark:bg-gray-800"
        >
        ...
        <select
          ...
          onChange={e => { setSelectedEstadoFilter(e.target.value); setCurrentPage(1); }}
          className="border p-2 rounded bg-white dark:bg-gray-800"
        >
            ...
            {pageItems.map((m, idx) => (
              ...
              >
                ...
                <td className="px-6 py-4 border space-x-2">
                  ...
                  {m.estado_Matricula !== 'AC' && m.estado_Matricula !== 'RE' &&
                    <button
                      onClick={() => handleUpdateEstado(m.id_Matricula,'AC')}
                      className="px-2 py-1 rounded bg-green-500 text-white hover:bg-green-600"
                    >
                      Aceptar
                    </button>
                  }
                  {m.estado_Matricula !== 'RE' && m.estado_Matricula!=='AC' &&
                    <button
                      onClick={() => handleUpdateEstado(m.id_Matricula,'RE')}
