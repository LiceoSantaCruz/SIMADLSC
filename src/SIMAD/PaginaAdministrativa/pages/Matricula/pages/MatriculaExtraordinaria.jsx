...
export const MatriculaExtraordinaria = () => {
  ...
  // Validación de campos obligatorios y formato antes de enviar el formulario
  const onSubmitHandler = async (e) => {
    e.preventDefault();
    const errors = [];

    // Página 1: Validaciones del estudiante
    if (!formData.periodo?.trim()) {
      errors.push("Periodo");
    }
    if (!formData.estudiante.gradoId?.toString().trim()) {
      errors.push("Grado");
    }
    if (!formData.estudiante.nombre_Estudiante?.trim()) {
      errors.push("Nombre del estudiante");
    }
    if (!formData.estudiante.apellido1_Estudiante?.trim()) {
      errors.push("Primer apellido del estudiante");
    }
    if (!formData.estudiante.cedula?.trim()) {
      errors.push("Cédula del estudiante");
    } else {
      const cedulaRegex = /^\d-\d{4}-\d{4}$/;
      if (!cedulaRegex.test(formData.estudiante.cedula.trim())) {
        errors.push("Cédula (formato válido: 5-0442-0911)");
      }
    }
    if (!formData.estudiante.correo_estudiantil?.trim()) {
      errors.push("Correo Estudiantil");
    } else {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(formData.estudiante.correo_estudiantil.trim())) {
        errors.push("Correo Estudiantil (debe ser un email válido)");
      }
    }
    if (formData.estudiante.fecha_nacimiento) {
      const birthDate = new Date(formData.estudiante.fecha_nacimiento);
      if (isNaN(birthDate.getTime())) {
        errors.push("Fecha de nacimiento (formato inválido)");
      }
    }
    // (Puedes agregar aquí más validaciones de la Página 1 si es necesario)

    // Por ejemplo, validación de Motivo de Matrícula (si es requerido en tu lógica)
    // if (!formData.estudiante.motivo_matricula?.trim()) {
    //   errors.push("Motivo de la Matrícula");
    // }

    // Si hay errores, se muestra un warning detallado
    if (errors.length > 0) {
      Swal.fire({
        icon: "warning",
        title: "Campos obligatorios incompletos o inválidos",
        text: "Por favor, corrija: " + errors.join(", "),
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 bg-white shadow-md">
      <h1 className="text-center text-2xl font-bold mb-2">
        Boleta de Matrícula Extraordinaria Año 2025
      </h1>
      <p className="text-center text-gray-600 mb-6">
        Por favor, complete todos los campos solicitados con datos verídicos.
        Verifique la información antes de enviar el formulario. Recuerde: la{" "}
        <strong>Cédula</strong> debe tener el formato{" "}
