...
export const EventList = () => {
  ...
  if (loading) {
    return <p className="text-center text-blue-600 dark:text-blue-300">Cargando eventos...</p>;
  }

  if (error) {
    return <p className="text-center text-red-500 dark:text-red-400">Error al cargar los eventos.</p>;
  }

  return (
    <div className="p-6 bg-gradient-to-r from-blue-50 to-pink-50 dark:from-gray-900 dark:to-gray-800 min-h-full transition-colors duration-300">
      ...
      {approvedEvents.length === 0 ? (
        <p className="text-center text-gray-500 dark:text-gray-400">
          No hay eventos aprobados por ahora.
        </p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {approvedEvents.map((event) => (
            <div
              key={event.id_Evento}
              className="p-6 border border-gray-200 dark:border-gray-700 rounded-xl shadow-md bg-white dark:bg-gray-900 flex flex-col"
            >
